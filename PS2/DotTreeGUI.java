...
public class DotTreeGUI extends DrawingGUI {
	...
	public void handleMousePress(int x, int y) {
		if (mode == 'a') {
			...
			//If tree is empty we add a new Dot
			//Else we insert a new Dot
			// TODO: YOUR CODE HERE
			if(tree==null) {
			...
			//If mode is q we change search for the points in a circle
			// TODO: YOUR CODE HERE
			mouseX=x;
			mouseY=y;
	...
	public void drawTree(Graphics g, PointQuadtree<Dot> tree, int level) {
		//Fix Oval at the intersection of two lines
		int offset = 5;
		// Set the color for this level
		g.setColor(rainbow[level % rainbow.length]);
		// Draw this node's dot and lines
		g.fillOval((int)tree.getPoint().getX()-offset,(int)tree.getPoint().getY()-offset,10,10);
